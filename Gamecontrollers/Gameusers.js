...
exports.getpearlusers = async (req, res) => {
    ...
    const pipeline = [
        ...
        // Group by referral ID
        {
            ...
            referralUsername: { $first: "$referralUser.username" },
  ...
  .then(data => {
    const result = data.map(item => {
      return {
        _id: item._id,
        referralUsername: item.referralUsername,
        pearls: item.pearls,
      };
    });

    const totalReferrals = result.length;

    // Calculate total pearls
    let totalPearls = 0;
    result.forEach(item => {
      totalPearls += item.pearls.length;
    });
